...
static constexpr auto genesis_hash =
...
int main() {
  ...
  auto print_chain = [&genesis_header](
    ...
    if (!start_from.has_value()) start_from = blocks.back().hash();
    ...
    while (!stk.empty()) {
      auto top = stk.top();
      stk.pop();
      if (visited_blocks.contains(top)) continue;
  ...
  auto get_blocks =
    [&](int id, const BlockHeader &header) -> ErrorOr<std::vector<Block>> {
    auto headers_response = TRY(TRY(manager.send(
                                      id,
                                      PeerMessage{
    ...
    auto headers_content =
      BlockHeadersResponseContent::from_json(headers_response.content());
    ...
    for (const auto &block_header: headers_content.headers()) {
      ...
      auto block_response = TRY(TRY(manager.send(
                                      id,
                                      PeerMessage{
  ...
  auto tip_from_specific_node = [&]() -> ErrorOr<void> {
    ...
    auto response = TRY(TRY(manager.send(
                              selected_peer,
                              PeerMessage{
  ...
  auto tip_from_all_nodes = [&]() -> ErrorOr<void> {
    auto responses =
      manager
        .broadcast(
          PeerMessage{PeerMessageType::GetLastBlockRequest, manager.id()}
        )
        .get();

...
namespace krapi {
  ...
  struct MiningJob {
    ...
    std::string obtain_merkle_root() {
      ...
      for (const auto &tx: m_transactions) { tree.insert(tx.hash()); }
   ...
   public:
    Miner(
      ...
      EventQueuePtr event_queue
        ...
        : m_event_queue(event_queue), m_worker(std::move(worker)) {}

    concurrencpp::shared_result<std::optional<Block>>
    mine(std::string previous_hash, std::set<Transaction> transactions) {

      return m_worker->submit(MiningJob{
        m_event_queue,
